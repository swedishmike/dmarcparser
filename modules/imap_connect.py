...
def connect_and_find_new_reports(hostname, username, password, target, deleteemails):
    ...
    try:
        ...
        logging.error(
            "Something went wrong when connecting to the IMAP server.", exc_info=True
        )
    ...
    try:
        ...
        print "\t[-] Connection to the IMAP server failed - check your settings"
        logging.error(
            "Something went wrong when connecting to IMAP server.", exc_info=True
        )
        ...
        for part in message.walk():
            ...
            if attach_name:
                ...
                if not (attach_name.endswith(".zip") or attach_name.endswith(".gz")):
                    logging.info(
                        'Ignoring non-zip or non-gz attachment "{0}"'.format(
                            attach_name
                        )
                    )
                    continue
                try:
                    ...
                    logging.error(
                        'Could not decode attachment "{0}"'.format(attach_name)
                    )
                    continue
